...
from apps.courses.serializers.category import (
...
class CategoryCreateView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response(
            CategoryModelSerializer(response).data, status=status.HTTP_201_CREATED
        )
...
class CategorySetIconView(GenericAPIView):
    ...
    def patch(self, request, category_id, *args, **kwargs):
        ...
        if not category_icon:
            return Response(
                {"detail": "Icon file doesnt provided"},
                status=status.HTTP_404_NOT_FOUND,
            )

        if category_icon.size > 1024 * 1024:
            return Response(
                {"detail": "Icon file too big"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        category.save()
        return Response(
            CategoryModelSerializer(category).data, status=status.HTTP_200_OK
        )
